...
export class H1transferComponent implements OnInit {
  ...
  @ViewChild(MatSort) sort!: MatSort;
  @ViewChild(MatPaginator) paginator!: MatPaginator;
  field = 'empty';               // Stores current search term or 'empty'
  sortField = 'updateddate';     // Default sort field
  ...
  //  Lifecycle Hooks
  ngOnInit(): void {
    const hasExcelPrivilege = this.privilegeServ.hasPrivilege('EXCEL_EXPORT');
    this.showReport = hasExcelPrivilege;
  ...
  //  API Call: Get All Data
  getAllData(pagIdx = 1) {
    //  Always send current filter + sort params
    this.consultantServ.getH1TransferList(
      pagIdx,
      this.pageSize,
      this.field,
      this.sortField,
  ...
  //  Generate Serial Number per page
  generateSerialNumber(index: number): number {
    const pagIdx = this.currentPageIndex === 0 ? 1 : this.currentPageIndex + 1;
  ...
  applyFilter(event: any) {
    ...
    //  Use same sort + pagination info when filtering
    this.consultantServ.getH1TransferList(
      1,
      this.pageSize,
      this.field,
  ...
  handlePageEvent(event: PageEvent) {
    ...
    //  Preserve search + sort params when changing page
    this.getAllData(event.pageIndex + 1);
  }
  ...
  onSort(event: Sort) {
    ...
    // Only fetch if direction exists (asc/desc)
    if (event.direction !== '') {
  ...
  // 🔹 Utility: Extract numeric value from phone numbers
  extractNumericValue(phoneNumber: string): number {
